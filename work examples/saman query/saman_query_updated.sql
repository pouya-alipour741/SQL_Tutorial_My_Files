...
select
	...
	,count(case when t.TaskStatusID = 6  then WorkflowInstanceID end) over(partition by Name) wf_count_open
	,count(case when t.TaskStatusID = 1  then WorkflowInstanceID end) over(partition by Name) wf_count_in_progress
